...
export function ProfileSheet() {
	...
	// Load user posts
	const { profilePostsData } = useProfilePosts(handleOrDid);
	// End Load user posts
		...
		profile && (
			<section className="relative">
				...
				<div className="absolute transform -translate-y-1/2 w-full flex justify-center">
					<UserAvatar
						className="w-24 h-24 border-4 border-white shadow-lg text-4xl"
						profile={profile}
					/>
				</div>
				...
				{profilePostsData.map((timelineEntry) => {
					return (
						<div key={`${timelineEntry.item.post.cid}`}>
							{(timelineEntry.item.post.record as AppBskyFeedPost.Record).text}
						</div>
					);
				})}
			</section>
