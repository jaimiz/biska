import { AppBskyFeedDefs, AppBskyFeedGetAuthorFeed, AppBskyFeedPost } from '@atproto/api';
...
export const useProfilePosts = (
	...
	const profilePostsQuery = useInfiniteQuery < Awaited < ReturnType < typeof fetchUserPosts >>> ({
	...
	const profilePostsData = useMemo(() => {
		...
		const flat = profilePostsQuery.data.pages.flatMap((page) => page.feed as AppBskyFeedDefs.FeedViewPost[]).filter((item) => {
			console.log({ item })
			return item !== undefined;
		});
		return flat
			.flatMap < TimelineEntry > ((item) => {
				return [{
					item: {
